...
func (c *cache[T]) GetWithExpiration(k string) (T, time.Time, bool) {
	...
	if !found {
		c.mu.RUnlock()
		return c.nilValueFunc(), time.Time{}, false
	}
...
func newCache[T any](de time.Duration, m map[string]Item[T], nilValue func() T) *cache[T] {
	if de == 0 {
		de = -1
	}
	c := &cache[T]{
		...
		items:             m,
		nilValueFunc:      nilValue,
	}
	return c
}

func newCacheWithJanitor[T any](de time.Duration, ci time.Duration, m map[string]Item[T], nilValue func() T) *Cache[T] {
	c := newCache(de, m, nilValue)
...
func New[T any](defaultExpiration, cleanupInterval time.Duration, nilValue func() T) *Cache[T] {
	items := make(map[string]Item[T])
	return newCacheWithJanitor(defaultExpiration, cleanupInterval, items, nilValue)
}
...
func NewFrom[T any](defaultExpiration, cleanupInterval time.Duration, items map[string]Item[T], nilValue func() T) *Cache[T] {
	return newCacheWithJanitor(defaultExpiration, cleanupInterval, items, nilValue)
}
